...
for (i in 1:ncol(y_train)) {
  model<-glm(y_train[,i]~X1+X4+X7+X10
               +X13+X16+X19
               +X22+X25+X28
               +X31+X34+X37
               +X40+X43
               ,family=poisson(link=log),data=data.frame(train_feature),maxit=100)
    
count_matrix[i,]<-as.vector(round(predict(model,data.frame(test_feature),type='response')))
